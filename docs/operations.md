...
`vista-move-engine-stack` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies engine behavior through framed sample traffic, with bounds and ordering tests and explicit failure cases.
...
The score is deterministic. The current threshold is 174. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.
